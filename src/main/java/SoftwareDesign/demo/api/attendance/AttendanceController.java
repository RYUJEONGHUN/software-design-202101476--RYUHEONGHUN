...
public class AttendanceController implements AttendanceApi {
    ...
    public ResponseEntity<ApiResponse<Long>> markSingleAttendance(@RequestBody AttendanceRequest request) {

        Long attendanceId = attendanceService.markAttendance(request);

        return ResponseEntity.ok(ApiResponse.success(SuccessCode.CREATE_SUCCESS, attendanceId));
    }

    @Override
    @PostMapping("/bulk")
    @PreAuthorize("hasRole('TEACHER')") // 대량 출석 처리
    public ResponseEntity<ApiResponse<List<Long>>> markBulkAttendance(
            @RequestBody List<AttendanceRequest> requests) {

        List<Long> attendanceIds = attendanceService.markBulkAttendance(requests);

        return ResponseEntity.ok(ApiResponse.success(SuccessCode.CREATE_SUCCESS, attendanceIds));
    }
